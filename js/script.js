...
function initMap() {
  ...
  c.onAdd = function () {
    ...
    d.innerHTML = '<div style="font-weight:600;margin-bottom:6px;">Kategori Prevalensi</div><div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;"><span style="width:12px;height:12px;background:#A1402F;display:inline-block;border-radius:2px;"></span> Tinggi (&ge;0,25/1.000)</div><div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;"><span style="width:12px;height:12px;background:#B9902E;display:inline-block;border-radius:2px;"></span> Sedang (0,18&ndash;0,25/1.000)</div><div style="display:flex;align-items:center;gap:6px;"><span style="width:12px;height:12px;background:#1F7A72;display:inline-block;border-radius:2px;"></span> Rendah (&lt;0,18/1.000)</div>';
    return d;
  };
  c.addTo(map);
}
...
function drawYear(year) {
  ...
  geoLayer = L.geoJSON(KECAMATAN_GEOJSON, {
    style: f => {
      const d = KECAMATAN_DATA.find(x => x.nama === f.properties.nama);
      return { fillColor: d ? catInfo(d[pKey]).color : '#CCCCCC', weight: 1.5, color: '#FFFFFF', fillOpacity: 0.75 };
    },
    onEachFeature: (f, l) => {
      const d = KECAMATAN_DATA.find(x => x.nama === f.properties.nama);
      if (!d) return;
      const info = catInfo(d[pKey]);
      l.bindPopup(`<b>${d.nama}</b><br>Tahun ${year}<br>Kasus: ${d[kKey]}<br>Penduduk: ${d.penduduk.toLocaleString('id-ID')}<br>Prevalensi: ${d[pKey]}/1.000<br>Kategori: ${info.label}`);
      l.on({ mouseover: e => e.target.setStyle({ weight: 3, color: '#155A54', fillOpacity: 0.9 }), mouseout: e => geoLayer.resetStyle(e.target), click: () => updateSidePanel(d, year) });
    },
  }).addTo(map);
...
function updateSidePanel(d, year) {
  ...
  if (d) {
    ...
    document.getElementById('spTotal').textContent = KECAMATAN_DATA.reduce((s, x) => s + x[kKey], 0);
  }
...
function renderTable() {
  const yf = document.getElementById('tableYear').value, year = yf.slice(1);
  const q = document.getElementById('searchBox').value.trim().toLowerCase();
  let rows = KECAMATAN_DATA.map(d => {
    const info = catInfo(d['p' + year]);
    return { nama: d.nama, penduduk: d.penduduk, __kasus: d[yf], __prev: d['p' + year], __cat: info.label, catCls: info.cls };
  });
  ...
  document.getElementById('tableBody').innerHTML = rows.map(r => `<tr><td>${r.nama}</td><td>${r.penduduk.toLocaleString('id-ID')}</td><td>${r.__kasus}</td><td>${r.__prev}</td><td><span class="pill ${r.catCls}">${r.__cat}</span></td></tr>`).join('');
}
